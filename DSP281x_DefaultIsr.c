...
interrupt void T1UFINT_ISR(void)   // EV-A
{
  ...
  // Remove after inserting ISR Code
	...
	IFR &= M_INT2; // Clear CPU Interrupt Flag 2
	...
	IER |= M_INT2; // Enable CPU Interrupt 2
	PieCtrlRegs.PIEACK.bit.ACK2 = 1; //Open INT2
	EINT; //Open Global INT
}
...
interrupt void T3UFINT_ISR(void)    // EV-B
{
  ...
  // Remove after inserting ISR Code
	...
	IFR &= M_INT4; // Clear CPU Interrupt Flag 4

	Spd_Test(); //Caculate Speed

	EvbRegs.EVBIFRA.bit.T3UFINT = 1;
	IER |= M_INT4; // Enable CPU Interrupt 4
	PieCtrlRegs.PIEACK.bit.ACK4 = 1; //Open INT4
	EINT;
}
